...
- **`BettingRoundCoordinator`** — Processes `PlayerCommand` (Fold/Call/Raise/AllIn) and determines when a betting round is complete. `BettingRoundState` tracks `toAct` (players who still owe an action), `actedSinceFullRaise` and `lastRaiseSize`; `settle()` ends the street when nobody owes an action (or no opponent can bet) and otherwise parks the turn on the next player in `toAct`, so `bettingPlayer()` never returns a 0-chip player during an active betting round — including right after the blinds (`PokerRound.start()` settles too, and can run a short-stacked hand straight to showdown). Raises follow no-limit rules: minimum raise = max(BB, last raise size); an all-in below a full raise doesn't reopen raising for players who already acted. Heads-up, the button posts the small blind and acts first pre-flop, last after the flop. `optionsFor()` exposes a player's call/min/max raise to clients.
- **Leaving / kicking mid-hand** — `PokerTable.kickPlayer` folds the player (out of turn via `PokerRound.forceFold`) and parks them in `PokerTableState.pendingRemovals`; they stay seated until `clearRoundState()`, because the live hand's pots and ordering reference them.
- **Pot results** (`round/PotResults.kt`) — `potWinners` is the single winner rule used by both the payout (`PokerRound.resolvePot`) and the table view; `PokerRoundState.potBreakdown()` lists main + side pots with contenders and, at a contested showdown, winners plus a `decidingReason` when hand names tie ("Queen kicker", "higher Sevens", "Split pot"). Sent as `GameStateUpdate.pots`.
- **Action log** — `PokerRoundState.actions` is the public `HandAction` log of the hand (blinds, fold/check/call/bet/raise/all-in, with amounts); recorded by `PokerRound`.
- **Computer players (`bot/`)** — `Player.botPersonality` (AGGRESSIVE = loose-aggressive, BALANCED = tight-aggressive, DEFENSIVE = rock) marks a bot. `BotView.from(round, bot)` is the information barrier — own pocket cards, face-up board, bets/stacks/positions and the action log only — and the sole input to `BotStrategy.decide`. Pre-flop uses `PreflopChart` percentiles (169 hand classes; regenerate via `PreflopChartGenerator`); post-flop uses `EquityCalculator` Monte Carlo (seeded from visible cards, so deterministic) against opponent ranges narrowed by `RangeModel` from their actions. The only randomness is one roll, in eligible spots, capped at 10% (`BotProfile.deviationRate`). `PokerTable.playBotTurn()` plays the current bot; bots are always ready and don't count toward `majorityIdle`. Tune profiles with the test-side `BotSimulationHarness` (VPIP/PFR/AF/steal), asserted by `BotPersonalityTuningTest`.
...
- **Computer players** — `CreateTableRequest.computerPlayers` seats bots at creation (personalities rotate). A bot's turn uses the same durable timer: `startTimer` schedules it after the bot's deterministic `thinkMs` (× `BOT_DELAY_SCALE`) and `onTimerFired` calls `playBotTurn()` instead of idling. Bots never vote (`eligibleVoters`).
...
- **Invite links** (`src/api/inviteLink.ts`) — `/table/:id` doubles as the join page (no session → `JoinForm`, which shows the table name and who's seated), so an invite is that absolute URL. The in-game table menu (`VotingMenu`) has "Copy invite link" (falls back to a selectable field without clipboard access); the home page's Join tab accepts a pasted link or a bare id (`parseTableRef`).
...
- **All-in runout** — `processFrame` in `PixiPokerTable.tsx` stages a frame that jumps to SHOWDOWN with undealt board cards: pocket cards are tabled, flop/turn/river are dealt with a 2s beat each, and only then do best hands, winners and payouts (`showdown`/`pot_awarded`/`chips_changed`) play; frames arriving meanwhile queue. `onRunoutChange` lets `ControlBar` hold Ready/Start ("Revealing the board…") while the canvas lags live state (runout or held frames). While it plays, each tabled hand shows its chance of winning (`runoutOdds` on the SHOWDOWN frame, computed server-side by `ShowdownOdds.forRunout` from the contenders' cards and the board shown so far — exact from the flop on, sampled pre-flop; folded hands stay unknown), stepping to each street's odds as its cards turn over (`Runout.boardShown` → `oddsAt`).
- **Pacing hold** — `processFrame` also queues warm frames in `SceneState.held` until `holdUntil`: a deal, a street being dealt, or an action's callout (`ACTION_BEAT_MS`) holds the next frame, so a bot's instant fold never lands mid-deal and consecutive actions play one at a time. Only the canvas waits; `ControlBar` reads live state. A cold frame flushes the queue.
- **Action callouts** (`burst.ts`) — bet/raise, ALL IN and FOLD each get a ring + label burst (`BurstTone`); folded seats stay dimmed until the next deal.
- **Pots** (`pot.ts`, `game/potResults.ts`) — one "POT" stack normally, a MAIN / SIDE row once an all-in splits it; after a showdown the result line(s) ("Ada wins — Queen kicker", "Split — Ada & Gus") sit under the pot (on phones, in place of its label) until the next deal, and payout chips fly from each pot to its own winners. Showdown halos come from `pots[].winnerIds`; the runout mask strips them until the board is out.
- **Chip piles** (`chipPile.ts`) — each seat's stack as clay chips on the felt (`CHIP_STADIUM`, slid `CHIP_ALONG` the rail), sized against the table's average stack; bets fly pile → pot (`spawnBetChips`), winnings pot → pile. The pot's chip count scales in big blinds.
...
**Mobile** — `useViewportMode` rotates the `GameScreen` stage 90° for a phone held in portrait (the stage's transform makes it the containing block for every fixed child), and on short screens (`compact`) the `ControlBar` becomes a right-hand column. `compact` also switches the Pixi scene to its phone layout (`SceneState.compact`): simplified card faces (one centred rank over suit, no corner indices), pocket-only showdown rows placed on the table side of each seat, badges beside the avatar, no bot-personality label, and a tighter crop centred on the table (`PHONE_CONTENT_W/H` in `doResize`). In dev builds `window.__pokerLayout()` reports seat bounds against the visible area; the `phone layout` e2e tests use it to assert a six-way showdown stays on screen.

**`ControlBar/` — In-round action bar:**
- Shows `Ready Up` / `Start Round` when no round is active.
- Shows `Fold` / `Call` (or `Check`) / `Raise` slider when a round is in progress **and** the local player has chips > 0. Players with 0 chips (all-in or eliminated mid-round) see a waiting label and no action buttons. The bottom bar has a fixed `min-height` so switching modes never re-fits the table.
...
| `BOT_DELAY_SCALE` | `1.0` | Multiplier on computer players' thinking delays (~0.7–2.6s). Also honored by `runLocal`. |
